...
namespace SOM
{
	...
	public class SOMAudioModule : SOMModule
	{
		...
		public override void Refresh()
		{
			...
			//SOMDataHandler.AddModule(rootMixersModule);
			...
			for (int i = 0; i < mixers.Count; i++)
			{
				...
				//SOMDataHandler.AddModule(mixerModule);
				if (addName)
					SOMDataHandler.AddConstant(mixerModule, "name", mixerName);
				...
				if (addSnapshots)
				{
					...
					//SOMDataHandler.AddModule(snapshotsModule);
		...
		void AddParameters(object audioMixer, string parentModule)
		{
			...
			for (int j = 0; j < properties.Length; j++)
			{
				...
				SOMDataHandler.AddConstant(parentModule, SOMUtils.NicifyConstantName(parameter), parameter);
			}
		}
		...
		void AddSnapshots(object audioMixer, string parentModule)
		{
			...
			for (int j = 0; j < snapshots.Length; j++)
			{
				...
				SOMDataHandler.AddConstant(parentModule, SOMUtils.NicifyConstantName(snapshot), snapshot);
			}
		}
